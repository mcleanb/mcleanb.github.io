...
public int patlength (int A[ ])
  {
	...
	public void doit(int A[])
	{
		...
		if (on_the_list(A))
			for(k=0;k<6;k++)
			doit(rotate(vflex(A),1,false));
		else add(A,bott);
	}


		public boolean on_the_list(int A[])
		{
			boolean there=false;
			int k=0,  j;
			while((k<bott) && (! there))
			   {
				   check=true; //assume kth one is it
				   for(j=0;j<6;j++)
				   {
					   rotate(A,1,false);
				       for (j=0;j<total;j++)
			                if (!(bunch[k][j]==A[j])) check =false;//but it wasn't
			           if (! check) there = check;
			       }
			   }
			return there;
		}

	    public void add(int A[], int bott)
			{
				int k;
