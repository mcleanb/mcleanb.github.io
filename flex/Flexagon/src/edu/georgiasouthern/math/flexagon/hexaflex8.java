...
//all works except V when upside down 
import javax.swing.*;
...
public class hexaflex8 extends JApplet
...
{     // reference variables
   ...
   /*    outs.addListSelectionListener(new ListSelectionListener() {
	public void valueChanged(ListSelectionEvent e) {
			...
			fillit(loc, face);
			hexagonPanel.showHexagon(state, attrib, loc, top);
     ...
     */  panel.add(outs, BorderLayout.CENTER);
       ...
       fill(face,attrib,4,0,0);fill(face,attrib,2,5,4);
	    ...
	    ///////// graphPanel.addVertex(fanswer);
	     fillit(loc, face);
			hexagonPanel.showHexagon(fanswer, attrib, loc, top);

    }//init
    public void see(boolean sh)
    {
    	   ...
    	   for(w=0;w<face.length;w++)
    		    {
    		     ...
    		     //outs.append(fanswer+" "+ canswer+pat_thick+"\n");
    			 if (sh) outListModel.add(outListModel.getSize(), fanswer+" "+ canswer+pat_thick);
    		     no_of_rotations =0;
    			 fInput.setText(fanswer);
    			 showStatus( "see" );
    			 canswer=" ";
    ...
    public void actionPerformed (ActionEvent e)
    {
      ...
      int loc[];
       loc = new int[6];
       int temp1[], temp2[]; temp1 = new int[total];temp2 = new int [total];
       
      if ( e.getSource( ) ==  seeButton)//upside down
      {
    	  ...
    	  for (f=0;f<total-1;f++) 
    	  {
    		  face[f] = temp1[total-f-2];//
    		  attrib[f] = temp2[total-f-2];
    	  }
     	  for (f=0;f<total-1;f++) 
     		  if (attrib[f]>-1) 
     			  {
     			  attrib[f]=(attrib[f]+10) % 20;
     			  if ((attrib[f]%10)==0)attrib[f]=attrib[f]+2; else if ((attrib[f]%10)==2)attrib[f]=attrib[f]-2;
     			  }
    	  rotate ( face, attrib, 2, true);
    	  top = !top;
      ...
      /*
		...
		fill(face,attrib,2,21,16);
         ...
         {
	    	 rotate(face,attrib,1,false);
			if ( vflex(face, attrib))
			
			 {
				 
				 rotate(face,attrib,4,false);
				 build_pat_thick(face, attrib);
				  see(true);
				 //create graph node (added by Emil)
			...
			else rotate(face,attrib,5,false);
...
public int patlength (int A[ ])
  {
	...
	public void rotate(int A[], int B[ ], int howmany, boolean last)
	{
	 ...
	 {
         int c[]; c=new int [total];
         int d[]; d=new int [total];
		 ...
		 {
		 	if (c[k]==0) sum = sum +1;
		 	if (c[k]==max-1) sum=sum+1;
		}
		 if (sum==2) //then there is a disconnect so raise some
			 for (k=0;k<len;k++) if (c[k] < c[total-2]) c[k]=c[k]+max;
		...
		} else
		{
			 ...
			 {
			 		while((j<len) && (c[i]>c[j])) j++;
			 	     //that unique spot where all numbers to left are larger
			 	     //than all numbers to the right th=0 means between 0 and others
			 		if (j<len)
			 		{
						 th=j;
